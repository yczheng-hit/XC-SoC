...
void UARTSIM::setup_listener(const int port)
{
	...
	if (m_skt >= 0)
		while (!check_for_new_connections())
		{
			printf("Listening on port %d\n", port);
			sleep(1);
		}
...
void UARTSIM::setup(unsigned isetup)
{
	...
	{
		m_setup = isetup;
		m_baud_counts_tx = (isetup & 0x0ffffff);
		m_baud_counts_rx = (isetup & 0x0ffffff);
...
void UARTSIM::setup(unsigned isetup_tx, unsigned isetup_rx)
{
	if (isetup_tx != m_setup)
	{
		m_setup = isetup_tx;
		m_baud_counts_tx = (isetup_tx & 0x0ffffff);
		m_baud_counts_rx = (isetup_rx & 0x0ffffff);
		m_nbits = 8 - ((isetup_tx >> 28) & 0x03);
		m_nstop = ((isetup_tx >> 27) & 1) + 1;
		m_nparity = (isetup_tx >> 26) & 1;
		m_fixdp = (isetup_tx >> 25) & 1;
		m_evenp = (isetup_tx >> 24) & 1;
	}
}
// }}}
...
// {{{
int UARTSIM::check_for_new_connections(void)
{
...
int UARTSIM::rawtick(const int i_tx, const bool network)
{
	...
	if (m_rx_state == RXIDLE)
	{
		...
		{
			m_rx_state = RXDATA;
			m_rx_baudcounter = m_baud_counts_rx + m_baud_counts_rx / 2 - 1;
	...
	else if (m_rx_baudcounter <= 0)
	{
		...
		m_rx_baudcounter = m_baud_counts_rx - 1;
	}
	else
		m_rx_baudcounter--;
	...
	if ((m_tx_state == TXIDLE) && ((network) || (m_conrd >= 0)))
	{
		...
		if (pb.revents & POLLIN)
		{
			...
			if (1 == nr)
			{
				...
				m_tx_state = TXDATA;
				o_rx = 0;
				m_tx_baudcounter = m_baud_counts_tx - 1;
			}
